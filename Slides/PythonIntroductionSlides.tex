...
\begin{frame}
  ...
  \begin{block}{What are They?}
    \begin{itemize}
      ...
      \item Numbers can be of the integer datatype, like \texttt{4}, or a floating-point datatype, like \texttt{4.0}).
      \item Text uses the string datatype, like \texttt{'Four score and seven years ago...'}.
...
\end{Verbatim}
        \normalsize
      \item Functions exist to coerce strings like \texttt{'1024'} into the corresponding floating-point or integer numbers (c.f. \texttt{int} and \texttt{float}), and so on.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{block}{Making Things Happen}
    \begin{itemize}
      \item \textbf{Methods} and \textbf{functions}\footnote{There is a distinction: methods are attached to and operate on their objects, while functions stand alone. Don't worry too much about this now, we'll come back to it later.} are procedures that work on variables to transform or otherwise alter them.
...
\begin{frame}[fragile]
  ...
  \begin{block}{A Few More Examples}
    \begin{itemize}
      \item Many of Python's datatypes support this sort of access (called indexing), which behind-the-scenes is just a regular method call:
\begin{Verbatim}[commandchars=\\\{\}]
...
\begin{frame}[fragile]
  ...
  \begin{block}{The \texttt{print} Function}
    \begin{itemize}
      \item Strings are surrounded by single (') or double (``) quotes, which are equivalent
  ...
  \frametitle{Functions}
    \begin{block}{Writing Your Own with \texttt{def}}
      \begin{itemize}
        ...
        \item Functions always \texttt{return} a value, which might be nothing (\texttt{None} in Python).
      \end{itemize}
\small
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c}{# This function has no parameters and...}
\PY{k}{def} \PY{n+nf}{test\PYZus{}func\PYZus{}1}\PY{p}{(}\PY{p}{)}\PY{p}{:}
  \PY{k}{return} \PY{l+s}{'}\PY{l+s}{returns a string.}\PY{l+s}{'}
\end{Verbatim}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{block}{Pig Latin}
    ...
    Then, \texttt{pig\_latinize('monster') = 'onstermay'}
  \end{block}
...
\begin{frame}[fragile]
  ...
  \begin{block}{Make Your Own Nonsense}
    \begin{itemize}
      ...
      \item You'll have to make one slice, access one element of the string directly, and then add 'ay' at the end:
...
\begin{frame}
  ...
  \begin{block}{Improving Pig Latinization}
    ...
    \begin{itemize}
      \item Modify your function so that it converts the pig latinized string to lowercase, then capitalizes it.
      \item You can do all of this in one line, but it is easier to read and understand if you store the intermediate pig latinized string in its own variables, like \texttt{pig\_latinized\_string}.
      \item Helpfully, you can 'chain' method calls, in other words, \texttt{some\_string.lower().capitalize()} works!
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{block}{Truth Testing}
    \begin{itemize}
      \item Few programs are just a straight pipeline of unbiased transformations to data -- most of the time, you'll want to branch out and make decisions based on input.
...
\begin{frame}[fragile]
  ...
  \begin{block}{Equality}
    \begin{itemize}
      \item You can also use \texttt{==}, \texttt{$<$}, \texttt{$<$=}, and so on.
